package com.example.lab.android.nuc.chat.utils.views;
...
public class RoundRectangleImageView extends HoverImageView {
    ...
    public RoundRectangleImageView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        setup(attrs);
    }

    public RoundRectangleImageView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void setup(AttributeSet attrs){
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.RoundRectangleImageView);

        //在HoverImageView的基础上添加了radius角度
        radius = typedArray.getDimension(R.styleable.RoundRectangleImageView_borderRadius,radius);
